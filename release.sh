...
rm -rf $outdir
mkdir -p $outdir $outdir/doc $outdir/tests $outdir/examples $outdir/compat

cp Makefile VERSION TODO Changelog readme.txt LICENSE \
   release.sh quickjs.h quickjs-libc.h \
   test262o.conf test262.conf \
   test262o_errors.txt test262_errors.txt \
   $outdir

cp -R src tools fuzz $outdir
cp compat/*.c $outdir/compat
cp tests/*.js tests/*.patch tests/*.c $outdir/tests
